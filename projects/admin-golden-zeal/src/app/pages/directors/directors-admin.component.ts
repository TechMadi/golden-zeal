...
@Component({
  ...
  template: `
    <div class="p-6 md:p-10 max-w-4xl">
      <h1 class="text-3xl mb-8" style="font-family:'Bebas Neue',sans-serif; color:#F0EBE0;">DIRECTORS</h1>
        ...
        @for (d of directors(); track d.id) {
          <div class="flex items-center justify-between p-4" style="background:#141414; border:1px solid rgba(240,235,224,0.07);">
            <div>
              ...
              <p class="text-xs" style="color:#888880;">{{ d.location }}</p>
            </div>
            <div class="flex gap-4">
              <button type="button" (click)="edit(d)" class="text-xs uppercase tracking-widest" style="color:#C9A04A;">Edit</button>
              <button type="button" (click)="delete(d.id)" class="text-xs uppercase tracking-widest" style="color:#888880;">Delete</button>
            </div>
          </div>
        }
      </div>
...
export class DirectorsAdminComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    obs.subscribe({
      next: () => { this.saving.set(false); this.saved.set(true); this.reset(); this.load(); setTimeout(() => this.saved.set(false), 2000); },
